...
const TableProductCategory = () => {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const result = await axios.delete(`https://syauqi-vps.my.id/api/canteen/v1/product-category/${id}`, {
        headers: {
                    ...
                    if (column.field === 'id') {
                      return (
                        <TableCell key={column.field}>
                          <Button onClick={() => handleDelete(value)}>Delete Product Category</Button>
                          <Link href={`/admin/product-category/${value}`} passHref>
